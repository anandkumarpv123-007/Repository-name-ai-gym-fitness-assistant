...
def generate_phase_10_docx():
    ...
    r_title = title.add_run("Phase 10 — Storage / Media / Infrastructure")
    r_title.font.name = 'Segoe UI Semibold'
    r_title.font.size = Pt(22)
    r_title.font.color.rgb = RGBColor(0x0D, 0x94, 0x88)  # Teal 600
    ...
    r_call_title = p_call.add_run("STORAGE & SUPPORTING INFRASTRUCTURE POLICY\n")
    r_call_title.bold = True
    r_call_title.font.size = Pt(10)
    r_call_title.font.color.rgb = RGBColor(0x0F, 0x76, 0x6E)  # Teal 700

    r_call_text = p_call.add_run(
        "All media files and binary storage assets are managed centrally through StorageService. "
        "Strict 50 MB limits, MIME type whitelisting, executable extension blocks, and path traversal protection "
        "are enforced across local filesystem, AWS S3 simulation, and Google Cloud Storage simulation provider modes. "
        "User authorization scoping guarantees isolation across user datasets."
    )
    ...
    md_path = os.path.join(os.path.dirname(__file__), "PHASE_10_MASTER_IMPLEMENTATION_GUIDE.md")
    if os.path.exists(md_path):
        ...
        for line in lines:
            ...
            if stripped.startswith("```"):
                if in_code_block:
                    # End of code block - render callout
    ...
    out_docx = os.path.join(os.path.dirname(__file__), "PHASE_10_MASTER_IMPLEMENTATION_GUIDE.docx")
    doc.save(out_docx)
    print(f"Successfully generated Phase 10 DOCX documentation: {out_docx}")
